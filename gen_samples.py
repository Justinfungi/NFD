...
def main():
    ...
    for i, image in enumerate(samples):
        ...
        avg_channels = np.mean(image, axis=2)

        save_dir = 'Examples'
        os.makedirs(save_dir, exist_ok=True)

        avg_image = avg_channels.astype(np.uint8)
        cv2.imwrite(os.path.join(save_dir, f'avg_{i}.png'), avg_image)

        first_32_channels = np.mean(image[:, :, :32], axis=2)
        first_32_image = first_32_channels.astype(np.uint8)
        cv2.imwrite(os.path.join(save_dir, f'first32_{i}.png'), first_32_image)

        first_32_channels = np.mean(image[:, :, 32:64], axis=2)
        first_32_image = first_32_channels.astype(np.uint8)
        cv2.imwrite(os.path.join(save_dir, f'second32_{i}.png'), first_32_image)

        first_32_channels = np.mean(image[:, :, 64:96], axis=2)
        first_32_image = first_32_channels.astype(np.uint8)
        cv2.imwrite(os.path.join(save_dir, f'thrid32_{i}.png'), first_32_image)

        for j in range(32):
            single = image[:, :,j]
            single = single.astype(np.uint8)
            cv2.imwrite(os.path.join(save_dir, f'{i}_{j}.png'), single)
        print(save_dir)
